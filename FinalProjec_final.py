# -*- coding: utf-8 -*-
"""
Created on Fri Apr 22 16:35:44 2016
...
"""
# input: FinalProject1(-1, 1, -1, lambda x: np.e**x, lambda x, y: x*np.e**(y*(1-x)))
# input: FinalProject1(0, np.pi, -1, lambda x: np.sin(10*x), lambda x, y: np.sin(x+y))
# input: FinalProject1(0, np.pi, -1, lambda x: 1+np.sin(np.pi*x), lambda x, y: x*np.cos(x*y))
def FinalProject1(a, b, lamda, psy0, K, problem):
    ...
    #return psy
        ...
        psy1 = psy_inter(a)
        psy2 = psy_inter(a+(b-a)/8)
        psy3 = psy_inter(a+(b-a)/4)
        psy4 = psy_inter(a+(b-a)/2)
        psy5 = psy_inter(b)
        psy_avg = (psy1+psy2+psy3+psy4+psy5)/5
        ...
        Error1 = Error
        if N != 100 and N != 200:
            #print tabulate([[i, psy_avg, Error, Convergence]], headers=["N", "psy_avg", "Error", "Convergence"])
