...
def ProjectTest2(H, k, g, target):
        ...
        def impact():
            for i in range(N-1):
                if y[i+1] - H1[i+1] <= 1e-6:
                    return i+1
                #else:
                 #   print("Impact point is out of range, change tRange")
        impact = impact()
        #plt.plot(x[0:impact], y[0:impact], 'r', x, H1, 'b')
        #print(target-x[impact])
        #return np.any(np.sqrt((H(target)-y[impact])**2 + (target-x[impact])**2))
    ...
    Project(H, theta, alpha, k, g, target)
